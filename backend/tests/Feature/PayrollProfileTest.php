...
class PayrollProfileTest extends TestCase
{
    ...
    public function test_a_profile_can_be_created_read_back_and_deleted(): void
    {
        ...
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload())
            ->assertCreated()
        ...
        $this->getJson("/api/auth/staff/{$staff->id}/payroll-profile")
            ->assertOk()
            ->assertJsonPath('data.account_number_masked', '••••••••9012');

        // A second write updates rather than creating a second row.
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload(['bank_name' => 'HDFC']))
            ->assertOk()
            ->assertJsonPath('data.bank_name', 'HDFC');
        $this->assertDatabaseCount('employee_payroll_profiles', 1);

        $this->deleteJson("/api/auth/staff/{$staff->id}/payroll-profile")->assertOk();
    ...
    public function test_the_account_number_is_never_returned_in_full(): void
    {
        ...
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload())->assertCreated();

        $response = $this->getJson("/api/auth/staff/{$staff->id}/payroll-profile")->assertOk();
    ...
    public function test_omitting_a_secret_keeps_it_while_an_empty_string_clears_it(): void
    {
        ...
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload(['account_number' => '']))
    ...
    public function test_changing_bank_details_is_logged_without_recording_them(): void
    {
        ...
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload())->assertCreated();
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload(['account_number' => '999988887777']))
            ->assertOk();
    ...
    public function test_an_employee_maintains_their_own_but_cannot_reach_a_colleagues(): void
    {
        ...
        $this->putJson("/api/auth/staff/{$mine->id}/payroll-profile", $this->payload())->assertCreated();
        $this->getJson("/api/auth/staff/{$mine->id}/payroll-profile")->assertOk();

        $this->getJson("/api/auth/staff/{$theirs->id}/payroll-profile")->assertForbidden();
        $this->putJson("/api/auth/staff/{$theirs->id}/payroll-profile", $this->payload())->assertForbidden();
        // Deleting one is an administrative act, not self-service.
        $this->deleteJson("/api/auth/staff/{$mine->id}/payroll-profile")->assertForbidden();
    }
    ...
    public function test_profiles_are_scoped_to_the_workspace(): void
    {
        ...
        $this->getJson("/api/auth/staff/{$staff->id}/payroll-profile")->assertForbidden();
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload())->assertForbidden();
    }
    ...
    public function test_input_is_validated(): void
    {
        ...
        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", ['country' => 'ZZ'])
            ->assertStatus(422)->assertJsonValidationErrors('country');

        $this->putJson("/api/auth/staff/{$staff->id}/payroll-profile", $this->payload(['swift_code' => str_repeat('X', 20)]))
