...
export function RegisterForm() {
  ...
  const onSubmit = async (values: RegisterValues) => { try { await registerUser(values); toast({ tone: "success", title: "Account created", description: "Welcome to Nucleus." }); router.replace("/dashboard") } catch (error) { toast({ tone: "error", title: "Unable to create account", description: getApiErrorMessage(error) }) } }
