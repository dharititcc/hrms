...
export function EmployeeFormDialog({ employee, onClose }: { employee?: Employee | null; onClose: () => void }) {
  ...
  const { register, control, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<EmployeeFormValues>({ resolver: zodResolver(employeeSchema), defaultValues: { name: employee?.name ?? "", email: employee?.email ?? "", phone: employee?.phone ?? "", role: employee?.role ?? "member", status: employee?.status ?? "active" } })
  useEffect(() => { reset({ name: employee?.name ?? "", email: employee?.email ?? "", phone: employee?.phone ?? "", role: employee?.role ?? "member", status: employee?.status ?? "active" }) }, [reset, employee])
